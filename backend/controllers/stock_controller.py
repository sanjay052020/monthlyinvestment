...
def add_multiple_stocks(data_list: list[StockCreate]):
    ...
    for data in data_list:
        ...
        record = {
            ...
            "price": data.price,
            "vendor_name": data.vendor_name,
            "date": data.date.isoformat()  # safer format
        }
        try:
            stocks.insert_one(record)
            results.append({"stock_id": stock_id, "name": data.productname})
        except Exception as e:
            results.append({"stock_id": stock_id, "error": str(e)})
    return {"message": f"{len(results)} stocks processed", "stocks": results}
